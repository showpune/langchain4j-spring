...
import static dev.langchain4j.local.spring.Properties.PREFIX;
...
public class AutoConfig {
    ...
    ContentRetriever contentRetriever(EmbeddingStore<TextSegment> embeddingStore, EmbeddingModel embeddingModel, Properties properties) {
        ...
        int maxResults = contentRetrieverProperties.getMaxResults() == null ? 1 : Integer.parseInt(contentRetrieverProperties.getMaxResults());
        double minScore = contentRetrieverProperties.getMinScore() == null ? 0.6 : Double.parseDouble(contentRetrieverProperties.getMinScore());
    ...
    @Bean
    @ConditionalOnProperty(name = PREFIX + ".content-retriver.use-local", havingValue = "true")
    EmbeddingStore<TextSegment> embeddingStore(EmbeddingModel embeddingModel, ResourceLoader resourceLoader, Properties properties)
            throws IOException {
        Properties.ContentRetrieverProperties contentRetrieverProperties = properties.getContentRetriver();
